...
export async function buildDailyReport(rawArgs = "", runtime: BuildDailyRuntime = {}): Promise<BuildDailyResult> {
	const options = parseDailyArgs(rawArgs, runtime.now || new Date());
	...
	const fallbackMarkdown = renderMarkdownReport(report);
	if (!runtime.ctx) {
		return { options, report, markdown: fallbackMarkdown, source: "fallback", errors: [] };
	}
	const aiResult = await summarizeReportWithAI(report, runtime.ctx);
